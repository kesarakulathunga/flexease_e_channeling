...
const { createSession, invalidateSession } = require('./sessionService');
...
exports.sendOtp = async (req, res, next) => {
  try {
    const { email, role, patientId, purpose = 'registration' } = req.body;
    ...
    await sendOtp(email, 'LOGIN', patientId || null, purpose);
...
exports.verifyEmailOtp = async (req, res, next) => {
  try {
    ...
    // Verify the OTP
    try {
      const verified = await verifyOtp(email, code, 'LOGIN', null, purpose);
      
      // Find existing accounts for this email
      const patients = await prisma.patientProfile.findMany({
        where: { email }
      });
      
      // Return verification status and existing accounts
      res.json({ 
        verified: true, 
...
exports.verifyOtp = async (req, res, next) => {
  try {
    const { email, code, role, patientId, purpose = 'registration' } = req.body;
    await verifyOtp(email, code, 'LOGIN', patientId || null, purpose);
...
exports.selectAccount = async (req, res, next) => {
  try {
    const { email, patientId, purpose = 'registration' } = req.body;
