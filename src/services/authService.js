...
const authService = {  /**
   ...
   * @param {string} purpose The purpose of checking email ('registration' or 'view_profile')
   * @returns {Promise<Object>} Response data
   */  
  checkEmail: async (email, isResend = false, purpose = 'registration') => {
    try {
      ...
      if (typeof isResend !== 'boolean') {
        console.warn(`Invalid isResend value: ${isResend}, defaulting to false`);
        isResend = false;
      }
      
      if (!purpose || (purpose !== 'registration' && purpose !== 'view_profile')) {
        console.warn(`Invalid purpose: ${purpose}, defaulting to 'registration'`);
        purpose = 'registration';
      }
      
      console.log(`Attempting to send OTP to ${email}${isResend ? ' (resend)' : ''} for purpose: ${purpose}`);
      
      // Prepare the request payload
      const payload = { email, isResend, purpose };
      ...
      return {
        ...
        purpose: purpose, // Include purpose in the response
   ...
   * @param {string} purpose The purpose of verification ('registration' or 'view_profile')
   ...
   */      verifyOTP: async (email, otp, purpose = 'registration') => {
    try {
      ...
      console.log(`Verifying OTP for ${emailTrimmed} with purpose: ${purpose}`);
      
      // Prepare the request payload with trimmed values
      const payload = { 
        email: emailTrimmed, 
        otp: otpTrimmed, 
        purpose 
      ...
      if (response.token) {
        ...
        // Dispatch custom event to notify AuthContext about the state change
   ...
   */    selectAccount: async (profileId, explicitEmail) => {
    try {      
      console.log(`Selecting profile with ID: ${profileId}`);
      
      // First try to use explicitly provided email (if provided)
      // Then try to get the email from localStorage or current user
  ...
  registerVerifiedPatient: async (patientData) => {
    try {
      ...
      if (response.profile) {
        ...
        localStorage.setItem('currentProfile', JSON.stringify(profile));      } else {
